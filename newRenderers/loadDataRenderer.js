...
function readFile(filepath) {
    fs.readFile(filepath, 'utf-8', (err, data) => {
        ...
        let loadedFileTextArea = document.getElementById('loadedFileTextArea');
        if (err) {
            loadedFileTextArea.innerHTML = "An error ocurred reading the file";
            loadedFileTextArea.setAttribute('class', 'text-danger');
            return
        }
        // var textArea = document.getElementById('output')
        // textArea.value = data

        loadedFileTextArea.innerHTML = "Loaded file: " + filepath;
        loadedFileTextArea.setAttribute('class', 'text-success');
